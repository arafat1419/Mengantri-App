...
class ServicesViewModel(private val dataUseCase: DataUseCase) : ViewModel() {
    ...
    fun getTicketServed(serviceId: Int): LiveData<Int> =
        dataUseCase.getTicketServed(serviceId).asLiveData()

    fun getServicesAndServed(companyId: Int): LiveData<List<ServiceCountDomain>> =
